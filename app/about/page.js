  ...
  return (
    <main className="about-page">
      ...
      <section className="about-visual page-width">
        ...
        <div className="about-message"><b>01</b><h2>{language === 'bg' ? 'Намираме стойността в добрата находка.' : 'We find value in a good fashion find.'}</h2><p>{language === 'bg' ? 'Когато финалният инвентар пристигне, всеки продукт ще показва реалната продажна цена, наличността, размерите и — когато е известна — оригиналната цена от етикета.' : 'When the final inventory arrives, each product will show its real selling price, availability, sizes and—where known—the original tag price.'}</p><Link href="/shop">{language === 'bg' ? 'Виж селекцията' : 'View selection'}<ArrowRight /></Link></div>
      </section>
