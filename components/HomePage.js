...
export default function HomePage() {
  ...
  const imageProducts = products.filter((product) => product.image && isProductAvailable(product));
  ...
  const latestProducts = [...products].sort((a, b) => (isProductAvailable(a) ? 0 : 1) - (isProductAvailable(b) ? 0 : 1)).slice(0, 8);
  const womenVisual = imageProducts.find((p) => productMatchesAudience(p, 'women'));
  const menVisual = imageProducts.find((p) => productMatchesAudience(p, 'men'));
    ...
    <main>
      ...
      <section className="campaign page-width">
        <div className="campaign-copy">
          ...
          <p>{t.home.saleText}</p>
          <Link href="/shop" className="campaign-link">{language === 'bg' ? 'Разгледай намаленията' : 'Explore the reductions'}<ArrowRight /></Link>
        </div>
        {heroProduct && (
      ...
      <section className="audience-banners page-width">
        <Link href="/women" className="audience-banner">
          {womenVisual?.image ? <Image src={womenVisual.image} alt="Women" fill sizes="33vw"/> : <div className="audience-placeholder" />}
          <span>{t.nav.women}<ArrowRight /></span>
        </Link>
        <Link href="/kids" className="audience-banner audience-banner-no-photo">
        ...
        <Link href="/men" className="audience-banner">
          {menVisual?.image ? <Image src={menVisual.image} alt="Men" fill sizes="33vw"/> : <div className="audience-placeholder"><b>GERPINA</b></div>}
