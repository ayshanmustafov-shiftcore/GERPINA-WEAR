...
export default function HomePage() {
  ...
  return <main>
    <section className="home-hero page-width">
      ...
      {heroProduct && <Link href={`/product/${heroProduct.slug}`} className="hero-panel hero-image-main"><PictureOrPlaceholder product={heroProduct} alt={heroProduct.name[language]} language={language} /><span className="hero-image-caption"><b>{heroProduct.brand}{heroDiscount ? ` · -${heroDiscount}%` : ''}</b><i><s>€{heroProduct.originalPrice.toFixed(2)}</s> €{heroProduct.price.toFixed(2)}</i></span></Link>}
      {sideProduct && <Link href={`/product/${sideProduct.slug}`} className="hero-panel hero-image-side"><PictureOrPlaceholder product={sideProduct} alt={sideProduct.name[language]} language={language} /><span className="hero-image-caption compact"><b>{sideDiscount ? `-${sideDiscount}%` : sideProduct.brand}</b><i><s>€{sideProduct.originalPrice.toFixed(2)}</s> €{sideProduct.price.toFixed(2)}</i></span></Link>}
    </section>
    ...
    <section className="campaign page-width"><div className="campaign-copy"><span>{language === 'bg' ? 'GERPINA SELECTION' : 'GERPINA SELECTION'}</span><h2>{t.home.saleTitle}</h2><p>{t.home.saleText}</p><Link href={scopedShopHref({ sale: '1' })} className="campaign-link">{language === 'bg' ? 'Разгледай предложенията' : 'Explore offers'}<ArrowRight /></Link></div>{heroProduct && <div className="campaign-price"><small>{heroProduct.originalPriceEstimated ? (language === 'bg' ? 'DEMO референтна цена' : 'DEMO reference price') : (language === 'bg' ? 'Референтна цена' : 'Reference price')}</small><div><s>€{heroProduct.originalPrice.toFixed(2)}</s><strong>€{heroProduct.price.toFixed(2)}</strong>{heroDiscount && <b>-{heroDiscount}%</b>}</div></div>}</section>
