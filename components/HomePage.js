...
export default function HomePage() {
  ...
  const scopedProducts = products.filter((product) => isProductAvailable(product) && productMatchesAudience(product, activeAudience));
  const imageProducts = scopedProducts.filter((product) => product.image);
  const allImageProducts = products.filter((product) => isProductAvailable(product) && product.image);
  ...
  const latestProducts = [...scopedProducts].slice(0, 8);
  ...
  return <main>
    <section className="home-hero page-width">
      ...
      {heroProduct && <Link href={`/product/${heroProduct.slug}`} className="hero-panel hero-image-main"><PictureOrPlaceholder product={heroProduct} alt={`${heroProduct.brand} ${heroProduct.name[language]}`} language={language} /><span className="hero-image-caption"><b>{heroProduct.brand}{heroDiscount ? ` · -${heroDiscount}% ${language === 'bg' ? 'спрямо цена на марката' : 'vs brand price'}` : ''}</b><i>{heroProduct.originalPrice ? <s>€{heroProduct.originalPrice.toFixed(2)}</s> : null} €{heroProduct.price.toFixed(2)}</i></span></Link>}
      {sideProduct && <Link href={`/product/${sideProduct.slug}`} className="hero-panel hero-image-side"><PictureOrPlaceholder product={sideProduct} alt={`${sideProduct.brand} ${sideProduct.name[language]}`} language={language} /><span className="hero-image-caption compact"><b>{sideDiscount ? `-${sideDiscount}% ${language === 'bg' ? 'спрямо цена на марката' : 'vs brand price'}` : sideProduct.brand}</b><i>{sideProduct.originalPrice ? <s>€{sideProduct.originalPrice.toFixed(2)}</s> : null} €{sideProduct.price.toFixed(2)}</i></span></Link>}
    </section>

    <section className="home-section page-width"><div className="section-title-row"><h2>{t.home.categories}</h2><Link href={scopedShopHref()}>{language === 'bg' ? 'Виж всички' : 'View all'} <ArrowRight /></Link></div><div className="category-tiles">{categoryKeys.map((category) => { const scopedVisual = imageProducts.find((product) => product.category === category); const visual = scopedVisual || allImageProducts.find((product) => product.category === category) || allImageProducts[0]; const targetAudience = scopedVisual ? activeAudience : visual?.audience === 'kids' ? 'kids' : visual?.audience === 'men' ? 'men' : 'women'; const href = `/shop?${new URLSearchParams({ audience: targetAudience, category }).toString()}`; return <Link href={href} className="category-tile" key={category}><div className="category-tile-image"><Image src={visual.image} alt={categoryLabels[category][language]} fill sizes="(max-width: 650px) 33vw, 16vw" /></div><strong>{categoryLabels[category][language]}</strong></Link>; })}</div></section>

    <section className="campaign page-width"><div className="campaign-copy"><span>{language === 'bg' ? 'GERPINA SELECTION' : 'GERPINA SELECTION'}</span><h2>{t.home.saleTitle}</h2><p>{t.home.saleText}</p><Link href={scopedShopHref({ sale: '1' })} className="campaign-link">{language === 'bg' ? 'Разгледай предложенията' : 'Explore offers'}<ArrowRight /></Link></div>{heroProduct && <div className="campaign-price"><small>{heroProduct.originalPrice ? (heroProduct.originalPriceType === 'reference' ? (language === 'bg' ? 'Референтна цена на марката' : 'Reference brand price') : (language === 'bg' ? 'Цена на марката' : 'Brand price')) : (language === 'bg' ? 'GERPINA цена' : 'GERPINA price')}</small><div>{heroProduct.originalPrice ? <s>€{heroProduct.originalPrice.toFixed(2)}</s> : null}<strong>€{heroProduct.price.toFixed(2)}</strong>{heroDiscount && <b>-{heroDiscount}%</b>}</div></div>}</section>
