...
function cmRange(label) {
  const value = String(label).trim().toLowerCase();
  const match = value.match(/(\d{2,3})\s*(?:\/\s*(\d{2,3}))?\s*(?:см|cm)/i);
...
export function compareSizes(a, b) {
  ...
  // Kids centimeter sizes in ascending order. Keep them before age labels
  // when a product exposes both measurements as separate selectable sizes.
  const aCm = cmRange(aLabel);
  const bCm = cmRange(bLabel);
  if (aCm || bCm) {
    if (!aCm) return 1;
    if (!bCm) return -1;
    return aCm.start - bCm.start || aCm.end - bCm.end || alphaFallback(aLabel, bLabel);
  }
