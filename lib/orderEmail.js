...
export function buildTestOrderEmail({ order, body }) {
  ...
  const text = [
    ...
    `Econt доставка на DEMO товарителницата: €${order.shippingPrice.toFixed(2)}`,
    `DEMO общо ориентировъчно: €${(order.merchandiseTotal + order.shippingPrice).toFixed(2)}`,
  ...
  <div style="font-family:Arial,sans-serif;max-width:680px;margin:auto;color:#111">
    ...
    <div style="padding:20px;border:1px solid #e5e5e5;border-top:0">
      ...
      <p><b>Продукти:</b> €${order.merchandiseTotal.toFixed(2)}<br>${typeof body.quotedShippingPrice === 'number' ? `<b>Реална checkout доставка:</b> €${Number(body.quotedShippingPrice).toFixed(2)}<br>` : ''}<b>Econt доставка (DEMO товарителница):</b> €${order.shippingPrice.toFixed(2)}<br><b>DEMO общо:</b> €${(order.merchandiseTotal + order.shippingPrice).toFixed(2)}</p>
