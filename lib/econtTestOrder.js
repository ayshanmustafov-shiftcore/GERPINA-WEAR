...
async function resolveDemoSenderOffice() {
  ...
  if (!office) throw new Error('Econt DEMO could not find the Полтава 3Ж sender office.');
  return {
...
async function resolveReceiverDestination(body) {
  ...
  if (body.deliveryType === 'office') {
    ...
    if (!office) throw new Error('The selected Econt office could not be matched in the DEMO system.');
    return { receiverOfficeCode: String(office.code), demoCity };
  }
