...
async function resolveDemoProfile() {
  // The public Econt DEMO credentials belong to a sample client profile. We only
  // use that profile for DEMO-only COD payout settings. Sender identity itself is
  // supplied explicitly as GERPINA so the generated test label does not show the
  // unrelated sample company from Econt's developer account.
  ...
  const senderPhone = cleanPhone(process.env.ECONT_SENDER_PHONE || '0877715366');
  const senderEmail = String(process.env.ORDER_FROM_EMAIL || 'orders@gerpina-wear.com').trim();
  const senderAgentName = String(process.env.ECONT_SENDER_AGENT_NAME || 'Петър Станиславов Петров').trim();

  const senderClient = {
    name: 'ГЕРПИНА УЕЪР ЕООД',
    phones: [senderPhone],
...
export async function createDemoEcontOrder(body, orderNumber) {
  ...
  const label = {
    ...
    sizeUnder60cm: true,
    shipmentDescription: '[TEST] Дрехи',
    orderNumber,
    services,
    payAfterAccept: true,
    payAfterTest: true,
