...
export function getEcontConfig() {
  return {
    mode: 'production',
    isProduction: true,
    createEnabled: String(process.env.ECONT_CREATE_ENABLED || '').trim().toLowerCase() === 'true',
    baseUrl: PRODUCTION_BASE_URL,
    username: String(process.env.ECONT_USERNAME || '').trim(),
    password: String(process.env.ECONT_PASSWORD || '').trim(),
...
export function getServerCart(items) {
  ...
  return items.map((item) => {
    ...
    let maxQuantity = Math.max(0, Number(product.stockQuantity) || 1);
    let inventoryNumbers = product.inventoryIds || [product.id];

    if (selectedSize && product.sizes?.length) {
      const variant = product.sizes.find((size) => size.label === selectedSize);
      if (!variant || variant.available === false || (variant.quantity != null && Number(variant.quantity) <= 0)) {
        throw new Error(`Selected size is no longer available for ${product.name?.en || product.id}.`);
      }
      if (variant.quantity != null) maxQuantity = Math.max(0, Number(variant.quantity) || 0);
      if (variant.inventoryIds?.length) inventoryNumbers = variant.inventoryIds;
    }

    if (maxQuantity <= 0) throw new Error(`${product.name?.en || product.id} is sold out.`);
    const quantity = Math.max(1, Math.min(Number(item.quantity) || 1, maxQuantity));
    return {
      id: product.id,
      quantity,
      price: Number(product.price),
      name: `${product.brand || ''} ${product.name?.bg || product.name?.en || product.id}`.trim(),
      selectedSize,
      inventoryNumbers: inventoryNumbers.slice(0, quantity),
    };
  });
}
...
export function getCartMetrics(items) {
  ...
  // Provisional checkout/waybill weight. GERPINA's current operational rule is to
  // prepare the waybill at 1 kg until the physical parcel is checked. The sender must
  // verify the real parcel before handing it to Econt and correct the waybill when it
  // is heavier. Exact product weights can replace this placeholder later.
  const shipmentWeightKg = 1;
...
function buildPackingList(cart, shipmentWeightKg) {
  const unitCount = cart.reduce((sum, item) => sum + item.quantity, 0);
  const unitWeightKg = Number((Math.max(1, Number(shipmentWeightKg) || 0) / Math.max(1, unitCount)).toFixed(6));

  return cart.map((item) => ({
    inventoryNum: (item.inventoryNumbers?.length ? item.inventoryNumbers : [item.id]).join(', '),
    description: `${item.name}${item.selectedSize ? ` · размер ${item.selectedSize}` : ''}`,
    // Econt stores a separate unit weight on every digital packing-list row.
    // Without this field e-Econt shows 0.000 kg even when the shipment-level
    // weight is present, and its barcode/PDF renderer can receive an incomplete
    // label record.
    weight: unitWeightKg,
    count: item.quantity,
    price: item.price,
...
export async function buildLabel({ items, deliveryType, city, officeCode, address, receiver }) {
  ...
  const safeShipmentWeightKg = Math.max(1, Number(shipmentWeightKg) || 0);
  ...
  if (!services.cdPayOptionsTemplate && !services.cdPayOptions) {
  ...
  const label = {
    ...
    packCount: 1,
    packs: [{ weight: safeShipmentWeightKg }],
    shipmentType: 'pack',
    weight: safeShipmentWeightKg,
    sizeUnder60cm: true,
    ...
    packingList: buildPackingList(cart, safeShipmentWeightKg),
  ...
  return {
    label,
    merchandiseTotal,
    shipmentWeightKg: safeShipmentWeightKg,
    setupMeta: {
      mode: config.mode,
        codSource: setup.cod.source,
      agreementConfigured: Boolean(config.cdAgreement),
