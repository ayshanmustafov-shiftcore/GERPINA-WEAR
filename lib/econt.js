...
let senderAddressCacheKey = '';
let senderAddressCacheAt = 0;
...
export function getEcontConfig() {
  ...
  return {
    ...
    // Actual GERPINA dispatch point. These are public business/logistics details, not secrets.
    // Vercel env values override the known GERPINA defaults below.
    senderCity: String(process.env.ECONT_SENDER_CITY || 'Велико Търново').trim(),
    senderPostCode: String(process.env.ECONT_SENDER_POSTCODE || '').trim(),
    senderQuarter: String(process.env.ECONT_SENDER_QUARTER || 'Кольо Фичето').trim(),
    senderStreet: String(process.env.ECONT_SENDER_STREET || 'Полтава').trim(),
    senderStreetNum: String(process.env.ECONT_SENDER_STREET_NUM || '3Ж').trim(),
...
async function resolveConfiguredSenderAddress(config, profile) {
  if (config.senderOfficeCode) return null;

  const hasConfiguredAddress = Boolean(
    config.senderCity &&
    config.senderStreet &&
    config.senderStreetNum
  );
  ...
  const cacheKey = [
    config.mode,
    config.senderCity,
    config.senderPostCode,
    config.senderQuarter,
    config.senderStreet,
    config.senderStreetNum,
  ].join('|');
  ...
  if (
    senderAddressCache &&
    senderAddressCacheKey === cacheKey &&
    now - senderAddressCacheAt < PROFILE_CACHE_MS
  ) {
    return senderAddressCache;
  }

  // Resolve the city through Econt nomenclatures so the API receives Econt's canonical
  // city ID/post code instead of relying on a hardcoded post code.
  const cityData = await econtRequest('Nomenclatures/NomenclaturesService.getCities.json', {
    countryCode: 'BGR',
  });

  const wantedCity = normalizeBg(config.senderCity);
  let cityMatches = (cityData?.cities || []).filter(
    (city) => normalizeBg(city.name) === wantedCity || normalizeBg(city.nameEn) === wantedCity
  );

  if (config.senderPostCode) {
    const postcodeMatches = cityMatches.filter(
      (city) => String(city.postCode || '') === config.senderPostCode
    );
    if (postcodeMatches.length) cityMatches = postcodeMatches;
  }

  if (!cityMatches.length) {
    throw new Error(`Econt could not find the configured sender city: ${config.senderCity}.`);
  }
  ...
  const rawAddress = {
    ...
    ...(config.senderQuarter ? { quarter: config.senderQuarter } : {}),
    street: config.senderStreet,
    num: config.senderStreetNum,
  };

  // Econt provides a dedicated address validator. Use its normalized response as the
  // sender address so calculate/validate and the future live-create request share the
  // exact same canonical dispatch point.
  const validation = await econtRequest('Nomenclatures/AddressService.validateAddress.json', {
    address: rawAddress,
  });

  const validatedAddress = validation?.address || rawAddress;
  senderAddressCache = validatedAddress;
  senderAddressCacheKey = cacheKey;
  senderAddressCacheAt = now;
...
export async function resolveAccountSetup() {
  ...
  const senderOfficeCode = config.senderOfficeCode || null;
  const senderAddress = senderOfficeCode ? null : await resolveConfiguredSenderAddress(config, profile);

  if (!senderOfficeCode && !senderAddress) {
    throw new Error('No sender office or address is configured for GERPINA.');
  }
...
export async function getSafeConnectionStatus() {
  ...
  try {
    ...
    return {
      ...base,
      profileLoaded: true,
      senderReady: Boolean(setup.senderOfficeCode || setup.senderAddress) && (!setup.senderIsJuridical || Boolean(setup.senderAgent?.name && setup.senderAgent?.phones?.length)),
...
export async function buildLabel({ items, deliveryType, city, officeCode, address, receiver }) {
  ...
  return {
    ...
    setupMeta: {
      ...
      codSource: setup.cod.source,
      agreementConfigured: Boolean(config.cdAgreement),
      senderAddressSource: setup.senderOfficeCode
        ? 'office'
        : (config.senderCity && config.senderStreet && config.senderStreetNum ? 'configured-address' : 'profile-address'),
    },
  };
}
